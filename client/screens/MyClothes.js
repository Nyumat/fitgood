...
export default function MyClothes({ navigation }) {
    ...
    const handleUpload = async (itemValue) => {
        setCategory(itemValue)
        const fileUri = image
        const fileName = fileUri.split('/').pop()
        try {
            let category = itemValue
            const response = await axios.post('http://192.168.10.27:3005/api/upload_item', {
    ...
    const getCategory = async () => {
        try {
            const response = await axios.get('http://192.168.10.27:3005/api/categories')
    ...
    return (
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
            ...
            <Box maxW="300" style={{ position: 'absolute', bottom: 105 }}>
                {image != null ? (
                    <Text
                        style={{
                            fontSize: 20,
                ...
                <Select
                    ...
                    mt={1}
                    onValueChange={(itemValue) => handleUpload(itemValue)}
                >
